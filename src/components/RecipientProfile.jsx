...
const RecipientProfile = ({ recipient }) => {
  ...
  async function handleTransection(data) {
    if (data.amount < 50) return toast.error("Amount must be more than 50");
    if (data.amount > sender.amount)
    ...
    try {
      ...
      if (confirmPin?.success) {
        const newTransection = {
          ...
          minusAmount: parseInt(data.amount) + (data.amount > 100 ? 5 : 0),
          sender: user.email,
  ...
  return (
    <div className="mt-3 space-y-3 text-center">
      ...
      <form onSubmit={handleSubmit(handleTransection)} className="">
