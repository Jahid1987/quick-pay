    ...
    <div className="pt-5 md:pt-8 w-full md:w-1/2 mx-auto">
      {!recipient ? (
        <form onSubmit={handleSubmit(handleRecipient)}>
          <h3 className="text-xl font-bold text-center">Pay Bill</h3>
      ...
      ) : recipient?.role !== "marchent" ? (
        <p className="text-red-400">
          You can pay bill only marchet.{" "}
          {`${recipient.name} is ${recipient.role}`}
        </p>
      ) : (
        <RecipientProfile transectionType="paybill" recipient={recipient} />
      )}
    </div>
  );
};
